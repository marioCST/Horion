...
void StorageESP::onPreRender(MinecraftUIRenderContext* renderCtx) {
	...
	for (const auto& chest : bufferedChestList) {
		...
		if (fill && (mode.selected == 0 || mode.selected == 3)) DrawUtils::draw2DBox(chest.lower, chest.upper, (float)fmax(0.5f, 1 / (float)fmax(1, Game.getLocalPlayer()->getRenderPositionComponent()->renderPos.dist(chest.lower))), true);
		switch (mode.selected) {
		case 0:
			DrawUtils::draw2DBox(chest.lower, chest.upper, (float)fmax(0.5f, 1 / (float)fmax(1, Game.getLocalPlayer()->getRenderPositionComponent()->renderPos.dist(chest.lower))));
			break;
		case 2:
			DrawUtils::drawBox(chest.lower, chest.upper, (float)fmax(0.5f, 1 / (float)fmax(1, Game.getLocalPlayer()->getRenderPositionComponent()->renderPos.dist(chest.lower))), fill, 2);
			break;
		case 3:
			DrawUtils::draw2DBox(chest.lower, chest.upper, (float)fmax(0.5f, 1 / (float)fmax(1, Game.getLocalPlayer()->getRenderPositionComponent()->renderPos.dist(chest.lower))), false, true);
			break;
		}
	}
}
