...
void Tracer::onLevelRender() {
	if (old) {
		...
		float calcYaw = (Game.getLocalPlayer()->getMovementProxy()->getRot().x + 90) * (PI / 180);
		float calcPitch = (Game.getLocalPlayer()->getMovementProxy()->getRot().y) * -(PI / 180);
		Vec3 moveVec;
	...
	if (!old) {
		Game.forEachEntity([&](Entity* ent, bool valid) {
			if (ent != Game.getLocalPlayer() && Target::isValidTarget(ent) && Game.canUseMoveKeys()) {
				...
				refdef2->OWorldToScreen(Game.getClientInstance()->levelRenderer->getOrigin(), ent->eyePos, target, Game.getClientInstance()->getFov(), screenSize);
