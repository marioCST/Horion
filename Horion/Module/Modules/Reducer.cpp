...
void Reducer::onTick(GameMode* gm) {
	...
	case 0:
		if (player->damageTime > 0 && player->getMovementProxy()->isOnGround()) player->jumpFromGround();
		break;
	case 1:
	...
	case 2:
		...
		if (player->damageTime > 0 && this->hitCount == 2 && player->getMovementProxy()->isOnGround()) {
