...
void NoFall::onSendPacket(Packet* packet) {
	...
	if (localPlayer->getMovementProxy()->getFallDistance() > 2.f && mode.selected == 0) {
	...
	if (mode.selected == 4) {
		...
		/*if (packet->isInstanceOf<C_MovePlayerPacket>() && !Game.getLocalPlayer()->getMovementProxy()->isOnGround()) { I don't know if this is better to have or not
			C_MovePlayerPacket* movePacket = reinterpret_cast<C_MovePlayerPacket*>(packet);
...
void NoFall::onTick(GameMode* gm) {
	LocalPlayer* localPlayer = Game.getLocalPlayer();
	if (localPlayer != nullptr) {
		if (localPlayer->getMovementProxy()->getFallDistance() > 2.f) {
			...
			case 4: {
				Vec3 blockBelow = localPlayer->getRenderPositionComponent()->renderPos;
